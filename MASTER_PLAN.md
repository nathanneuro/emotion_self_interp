...
| Phase 4 α-sweep on Monet-4.1B (sparse-MoE) | **done** (2026-04-29) | Messy non-monotonic on negative arm (reverses at α=−2); positive arm monotonic; capability robust across α=[−2,+2] (sub↔Likert +0.25, loose). Cross-paradigm Phase 4 complete: **only universal-transformer base shows clean response**; standard-T base and sparse-MoE base both show messy. Looping is the architectural property that develops tight substrate↔readout coupling in pretraining |
| Phase 4 α-sweep on Qwen-0.5B base | **done** (2026-04-29) | **Both arms** non-monotonic (messiest in suite; sub↔Likert +0.45, just below cliff). **Same model, base→Instruct shifts coupling from +0.45 to +0.52, flips α-response from messy to clean.** Cliff is around sub↔Likert +0.5; std-T base / sparse-MoE base sit below it; post-training pushes std-T above; universal-T is born above |
| Within-arch scaling: Ouro-2.6B base | **done** (2026-04-29) | 48 layers × 4 ut = 192 events/forward (vs 1.4B's 96). Likert r vs target jumps to **+0.751** (top of suite, beating all post-trained models). 6-class adapter accuracy 0.55 (top of suite). α=0 calm/eu +1.31 — model has internalized valence *signs*. Clean monotonic α-response within \|α\|≤1. Universal-T mechanism compounds with depth |
| Per-ut Likert on Ouro-2.6B | **done** (2026-04-29) | Peak shifts from ut=2 (1.4B) to **ut=3** (2.6B); peak magnitude tightens to +0.751 (no compression). ut=0 weaker (+0.23) due to deeper stack needing more iterations to develop. **Phase transition: blissful at ut=3 turns POSITIVE (+0.40) — only at 2.6B, not at 1.4B-Thinking which saturated negative**. Adds calibration-quality axis: depth unlocks valence-sign calibration |
| RWKV-7 partial Exp 1 v1 | **done** (2026-04-29) | At L20 (per-stimulus optimal, vs Phase 1 PCA peak L7): substrate r=**+0.84** (top of suite!), Likert +0.64, sub↔Likert **+0.71**. Custom RWKV Likert scorer using recurrent state for teacher-forced rating tokens. **RWKV (recurrent linear-attention) base develops tight substrate↔readout coupling in pretraining alone — like universal-T**. Generalizes the architectural mechanism: any recurrent paradigm applies readout machinery many times per stimulus during pretraining |
| State-conditional substrate on RWKV-7 | **done** (2026-04-29) | Primed state with calm/desperate/neutral context, then extracted substrate + Likert. Priming valence direction shifts readouts as predicted (calm>neutral>desperate on both substrate-val mean and Likert mean). **Surprise: any priming shifts Likert mean strongly negative** — "prior context exists" effect dominates absolute magnitudes, valence direction is second-order. Substrate r vs target *degrades* with priming (per-stimulus discrimination reduces); Likert r vs target *increases*. Unique-to-recurrent interpretive surface that doesn't apply to transformers |
| State decay on RWKV-7 | **done** (2026-04-29) | Substrate priming-direction signal persists across 50 filler sentences (~750 tokens, Δ +0.04 calm vs desperate). **Likert valence-direction effect decays within ~50 sentences.** "Prior context exists" Likert shift kicks in within 5 filler tokens — pure context-exists effect, not content-driven. Substrate and Likert have qualitatively different state-dependence dynamics; priming-as-steering works on substrate, fails on Likert over >tens-of-sentences |
